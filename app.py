...
webdav_options = {
    'webdav_hostname': "https://u441636.your-storagebox.de",  # Votre serveur WebDAV
    'webdav_login': "u441636",                                 # Votre identifiant
...
class Post(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    # Chemin distant sur le serveur WebDAV où le contenu du post est stocké
...
@app.route('/')
def index():
    # Pour le débogage, afficher TOUS les posts (pour tous les utilisateurs)
    posts = Post.query.options(joinedload(Post.author)).order_by(Post.timestamp.desc()).all()
    # Pour chaque post, télécharger le contenu depuis le serveur WebDAV
    for post in posts:
        local_temp_path = f"temp_{post.id}.txt"
        content = ""
        try:
            webdav_client.download_sync(remote_path=post.content_file, local_path=local_temp_path)
...
def new_post():
    if request.method == 'POST':
        ...
        filename = f"posts/{current_user.id}_{timestamp}.txt"
        ...
        local_temp_path = f"temp_{current_user.id}_{timestamp}.txt"
